...
using Real_Time_Screen_Filters.Properties;
...
namespace Real_Time_Screen_Filters
{
    public partial class Ventana_Principal : Form
    {
        ...
        internal static bool Variable_Dibujar_Cursor = true;
        ...
        internal readonly string Texto_Título = "Real Time Screen Filters by Jupisoft for " + Program.Texto_Usuario;
        ...
        private void Ventana_Principal_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Menú_Contextual_Mover_Ventana_Arriba_Derecha.PerformClick();
        ...
        private void Menú_Contextual_Negativo_Anterior_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Menú_Contextual_Dibujar_Cursor_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void Menú_Contextual_Guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Picture.Image != null)
                {
                    Program.Crear_Carpetas(Ruta_Guardado);
                    Picture.Image.Save(Ruta_Guardado + "\\" + Program.Obtener_Nombre_Temporal() + ".png", ImageFormat.Png);
                    SystemSounds.Asterisk.Play();
                }
                Temporizador_Principal.Start();
            }
                    ...
                    //if (Variable_Dibujar_Espaciado_FPS)
                    {
                        ...
                        if (Ancho_FPS > 0) // Don't draw if the window is minimized.
                        {
                            ...
                            foreach (int Milisegundo in Lista_FPS_Milisegundos)
                            {
                                ...
                                Pintar_FPS.FillRectangle(Pincel, ((Milisegundo * Ancho_FPS) / 999), 0, 1, 8);
                                Pincel.Dispose();
                                Pincel = null;
                            }
                            Pintar_FPS.Dispose();
                        ...
                        (Control.ModifierKeys & Keys.Control) != Keys.Control)
                    {
                        ...
                        if (Ancho > 0 && Alto > 0)
                        {
                            ...
                            using (Graphics gsrc = Graphics.FromHwnd(IntPtr.Zero)) // Desktop window?
                            {
                                ...
                                int retval = PInvoke.Gdi32.BitBlt(hDC, 0, 0, Ancho, Alto, hSrcDC, Rectángulo.X, Rectángulo.Y, !Variable_Negativo ? (int)CopyPixelOperation.SourceCopy : (int)CopyPixelOperation.NotSourceCopy);
                                Pintar.ReleaseHdc();
                                gsrc.ReleaseHdc();
                            }
        ...
        /// <summary>
        /// Function that filters an image with the desired filter.
        /// </summary>
        /// <param name="Imagen">Any valid bitmap with either</param>
                ...
                if (Imagen != null)
                {
                    ...
                    else if (Filtro != Filtros.Original && (Filtro < Filtros.Flip_horizontally || Filtro > Filtros.Rotate_180_degrees))
                    {
                        ...
                        else if (Filtro >= Filtros.Posterize_2_tones && Filtro <= Filtros.Posterize_128_tones)
                        {
                            ...
                            if (Filtro == Filtros.Posterize_2_tones) Posterizado = 2;
                            ...
                            else if (Filtro == Filtros.Posterize_128_tones) Posterizado = 128;
                            int Diferencia = 256 / Posterizado;
                            int Diferencia_Media = ((256 / Posterizado) / 2) - 1;
                            byte[] Matriz_Posterizado = new byte[256];
                            for (int Índice = 0; Índice < 256; Índice++)
                            {
                                int Valor = (int)Math.Round(((double)(Índice / (256 / Posterizado)) * 255d) / ((double)Posterizado - 1d), MidpointRounding.AwayFromZero);
                                //int Valor = ((Índice / (256 / Posterizado)) * 255) / (Posterizado - 1);
                                //int Valor = ((Índice % Posterizado) * Diferencia) + Diferencia_Media;
                                if (Valor < 0) Valor = 0;
                                else if (Valor > 255) Valor = 255;
                                Matriz_Posterizado[Índice] = (byte)Valor;
                            }
                            Program.Copiar_Texto_Portapapeles(Program.Traducir_Matriz_Bytes(Matriz_Posterizado));
                            for (int Índice_Y = 0, Índice = 0; Índice_Y < Alto; Índice_Y++, Índice += Bytes_Diferencia)
                            {
                                for (int Índice_X = 0; Índice_X < Ancho; Índice_X++, Índice += Bytes_Aumento)
                                {
                            ...
                            byte[] Matriz_Posterizado = null;
                            if (Filtro == Filtros.Posterize_2_tones) Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_2.Clone();
                            else if (Filtro == Filtros.Posterize_4_tones) Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_4.Clone();
                            else if (Filtro == Filtros.Posterize_8_tones) Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_8.Clone();
                            else if (Filtro == Filtros.Posterize_16_tones) Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_16.Clone();
                            else if (Filtro == Filtros.Posterize_32_tones) Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_32.Clone();
                            else if (Filtro == Filtros.Posterize_64_tones) Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_64.Clone();
                            else Matriz_Posterizado = (byte[])Program.Matriz_Bytes_Filtro_Posterizar_128.Clone();
                            for (int Índice_Y = 0, Índice = 0; Índice_Y < Alto; Índice_Y++, Índice += Bytes_Diferencia)
                            {
                                for (int Índice_X = 0; Índice_X < Ancho; Índice_X++, Índice += Bytes_Aumento)
                                {
        ...
        private void Menú_Contextual_Mover_Ventana_Arriba_Izquierda_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Arriba_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point((Rectángulo_Pantalla.X + (Rectángulo_Pantalla.Width / 2)) - (this.Width / 2), Rectángulo_Pantalla.Y);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Arriba_Derecha_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point(Rectángulo_Pantalla.Right - this.Width, Rectángulo_Pantalla.Y);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Izquierda_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point(Rectángulo_Pantalla.X, (Rectángulo_Pantalla.Y + (Rectángulo_Pantalla.Height / 2)) - (this.Height / 2));
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Centro_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point((Rectángulo_Pantalla.X + (Rectángulo_Pantalla.Width / 2)) - (this.Width / 2), (Rectángulo_Pantalla.Y + (Rectángulo_Pantalla.Height / 2)) - (this.Height / 2));
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Derecha_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point(Rectángulo_Pantalla.Right - this.Width, (Rectángulo_Pantalla.Y + (Rectángulo_Pantalla.Height / 2)) - (this.Height / 2));
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Abajo_Izquierda_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Bottom - this.Height);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Abajo_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point((Rectángulo_Pantalla.X + (Rectángulo_Pantalla.Width / 2)) - (this.Width / 2), Rectángulo_Pantalla.Bottom - this.Height);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }

        private void Menú_Contextual_Mover_Ventana_Abajo_Derecha_Click(object sender, EventArgs e)
        {
            try
            {
                Rectangle Rectángulo_Pantalla = Screen.PrimaryScreen.WorkingArea;
                Rectángulo_Pantalla = new Rectangle(Rectángulo_Pantalla.X, Rectángulo_Pantalla.Y + SystemInformation.CaptionHeight, Rectángulo_Pantalla.Width, Rectángulo_Pantalla.Height - SystemInformation.CaptionHeight);
                this.Location = new Point(Rectángulo_Pantalla.Right - this.Width, Rectángulo_Pantalla.Bottom - this.Height);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }
    }
